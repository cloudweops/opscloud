<template>
  <div class="container">
    <Breadcrumb :items="['menu.mcenter.user', 'menu.mcenter.user.list']" />
    <a-card class="general-card" :title="$t('menu.mcenter.user.list')">
      <a-row>
        <a-col :flex="1">
      ...
      <a-table row-key="id" :loading="loading" :pagination="pagination" :columns="(cloneColumns as TableColumnData[])"
        ...
        <template #index="{ rowIndex }">
          {{ rowIndex + 1 + (pagination.pageNumber - 1) * pagination.pageSize }}
        </template>
...
import { queryMcenterUserSet, queryMcenterUserReq, McenterUserSet } from '@/api/user-center';
import { Pagination, McenterPagination } from '@/types/global';
...
const { t } = useI18n();
const renderData = ref<McenterUserSet[]>([]);
...
const basePagination: McenterPagination = {
  pageNumber: 1,
  pageSize: 20,
};
...
const fetchData = async (
  ...
  setLoading(true);
  try {
    const { data } = await queryMcenterUserSet(params);
